...
$res=DB::paginate("SELECT * FROM trains WHERE ".implode(' AND ',$w)." ORDER BY id ASC",$p,$pg);
$pageTitle='Train Tickets — TravelNest';
...
<div class="sec">
  <h2 class="stitle">Train Tickets</h2><p class="ssub"><?=$res['total']?> available</p>
  ...
  <?php foreach($res['data'] as $item):
    ...
    $fn='show'.ucfirst(rtrim('trains','s'));
  ?>
  <div class="card mb10 flex g16" style="cursor:pointer" onclick="<?=$fn?>(<?=$item['id']?>)">
    ...
    <div class="tr" style="min-width:150px;display:flex;flex-direction:column;justify-content:space-between;gap:10px">
      ...
      <div class="flex g4r" style="justify-content:flex-end">
        ...
        <a href="<?=BASE?>/book.php?type=<?=rtrim('trains','s')?>&id=<?=$item['id']?>" class="btn btn-primary btn-xs" onclick="event.stopPropagation()">Book</a>
      </div>
    </div>
  </div>
  <?php endforeach;?>
  <?=pagLinks($res['page'],$res['last'],BASE."/trains.php?q=".urlencode($q))?>
</div>

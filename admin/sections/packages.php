...
$res=DB::paginate("SELECT * FROM packages WHERE ".implode(' AND ',$w)." ORDER BY id ASC",$p,$pg,15);
$base=BASE."/admin/index.php?sec=packages&q=".urlencode($q);
?>
<div class="flex sb wrap-x g12 mb20"><h2 style="font-size:22px"><?=ucfirst('packages')?> <span class="bdg t-blue"><?=$res['total']?></span></h2></div>
<div class="card">
<form method="GET" class="fbar mb12"><input type="hidden" name="sec" value="packages">
...
?>
<tr data-type="<?=rtrim('packages','s')?>" data-id="<?=$item['id']?>">
  ...
  <td><button class="btn btn-<?=$item['is_active']?'danger':'green'?> btn-xs" onclick="toggleStatus('<?=rtrim('packages','s')?>',<?=$item['id']?>,<?=$item['is_active']?>)"><?=$item['is_active']?'Disable':'Enable'?></button></td>
</tr>
<?php endforeach;?>
</tbody></table></div>
